...
- `editmode`: `1` (editor surface — panorama clicks are posted to the parent as `panoramaClick`)

In edit mode, navigation-type hotspot clicks do **not** change scene — the host must drive scene changes via the inbound `goToScene` command. Hotspot markers look draggable in the viewer UI, but drags are not persisted; the host listens for `panoramaClick` and handles placement/persistence itself.
...
- `panoramaClick` (only with `editmode=1`; payload: `yaw`, `pitch` in degrees, `sceneId`)
