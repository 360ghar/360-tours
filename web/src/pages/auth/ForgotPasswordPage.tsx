...
import { authApi, checkIdentifierStatusSafe } from '@/api';
...
import { mapSupabaseAuthError, IDENTIFIER_STATUS_UNAVAILABLE_MESSAGE } from '@/lib/authErrors';
  ...
  const onRequestOTP = async (data: { identifier: string }) => {
    ...
    try {
      ...
      if ('unavailable' in status) {
        // Identifier-status is required to decide reset vs no-account. A
        // network/5xx failure must not look like "no account" or trigger OTP.
