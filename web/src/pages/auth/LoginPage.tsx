...
import { checkIdentifierStatusSafe, type IdentifierChannel } from '@/api';
import { stashOAuthNext, supabaseAuth } from '@/lib/supabaseAuth';
...
export function LoginPage() {
  ...
  const onSubmitIdentifier = async (data: IdentifierFormData) => {
    ...
    setStepLoading(true);
    try {
      const status = await checkIdentifierStatusSafe(data.identifier);
      if ('unavailable' in status) {
        // Never fall through to OTP/signup when identifier-status is down —
        // that risks silent account creation for existing users.
        setLocalError(IDENTIFIER_STATUS_UNAVAILABLE_MESSAGE);
        return;
      }
      ...
      setLocalError(
        err instanceof Error ? mapSupabaseAuthError(err) : 'Could not continue. Please try again.'
      );
    } finally {
  ...
  const onGoogle = async () => {
    ...
    try {
      // Stash return path client-side; redirectTo stays clean (no ?next=)
      // so Supabase allowlist matching does not fall back to Site URL.
      stashOAuthNext(from !== ROUTES.DASHBOARD ? from : null, ROUTES.DASHBOARD);
